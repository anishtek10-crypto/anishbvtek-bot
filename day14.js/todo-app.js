...
async function addNote(title, content) {
    ...
    console.log('yes Note added:', newNote);
}
...
async function getNote(id) {
    ...
    if (!note) return console.log('No Note not found');
...
async function run() {
    ...
    switch (command) {
        ...
        default:
            console.log(`\nUsage:
    node todo-app.js add "Title" "Content"
    node todo-app.js list
    node todo-app.js get <id>
    node todo-app.js update <id> "New Content"
    node todo-app.js delete <id>\n`);
    }
} 
run();